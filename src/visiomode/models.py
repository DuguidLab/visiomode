...
class Protocol(Base):
    ...
    Attributes:
        ...
        created_by (str): Name of the individual who created the protocol.
        created_on (str): ISO date of when protocol was created.
        last_modified (str): ISO date of when protocol was last modified.
        id (str): Autogenerated ID for the protocol.
        protocol_spec (dict): A dictionary containing all info needed to run a protocol. This is equivalent
            to the json dict that would be passed to Visiomode at runtime to start a new session.
    """
    ...
    protocol_spec: dict = dataclasses.field(default_factory=dict)
    _db_path = f"{cfg.db_dir}{os.sep}protocols.json"

    def __post_init__(self):
        # Generate ID if class was instantiated with id=None
        if not self.id:
